...
df_consent = df_patients.withColumn("consent_given", F.lit(True)) \
    ...
    .withColumn("erasure_date", F.when(F.col("erasure_requested") == True,F.lit("2024-06-01").cast(DateType())).otherwise(F.lit(None).cast(DateType())))\
