...
#patients_deid.write.mode("overwrite").parquet("s3://clinicalflow-datalake-941141114246/silver/patients/")
    
patients_deid.writeTo("glue_catalog.clinicalflow_silver.patients") \
...
#Batch (what's in your script):
#df = spark.read.parquet("s3://...")          # reads once, all data
#df.write.mode("overwrite").parquet("s3://...") # writes once, done

#Streaming:
#df = spark.readStream.parquet("s3://...")         # watches for new files continuously
